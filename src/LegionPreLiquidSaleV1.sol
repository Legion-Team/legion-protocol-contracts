...
contract LegionPreLiquidSaleV1 is ILegionPreLiquidSaleV1, LegionVestingManager, Initializable, Pausable {
        ...
        whenNotPaused
    {
        ...
        _updateInvestorPosition(investAmount, tokenAllocationRate, investorVestingConfig);
        ...
        askTokenAvailable
    {
        ...
        uint256 amountToDistributeOnClaim = totalAmount * position.investorVestingConfig.tokenAllocationOnTGERate / 1e18;
        ...
        if (amountToBeVested != 0) {
            ...
            address payable vestingAddress = _createVesting(position.investorVestingConfig);

            /// Save the vesting address for the investor
            position.investorVestingConfig.vestingAddress = vestingAddress;
    ...
    function releaseVestedTokens() external whenNotPaused askTokenAvailable {
        ...
        if (position.investorVestingConfig.vestingAddress == address(0)) revert Errors.ZeroAddressProvided();

        /// Release tokens to the investor account
        ILegionLinearVesting(position.investorVestingConfig.vestingAddress).release(saleStatus.askToken);
    }
    ...
    {
        /// Get the investor position details
        address investorVestingAddress = investorPositions[investor].investorVestingConfig.vestingAddress;
        ...
        virtual
    {
        ...
        /// Compare the hashed vesting configuration with the current one and update if different
        if (keccak256(abi.encode(position.investorVestingConfig)) != keccak256(abi.encode(investorVestingConfig))) {
            position.investorVestingConfig = investorVestingConfig;
        }
    }
    ...
    function _verifyValidPosition(bytes memory signature, SaleAction actionType) internal view {
        ...
        bytes32 _data = keccak256(
            abi.encode(
                ...
                position.investorVestingConfig,
                actionType
            )
